...
class Vertex
  ...
  public:
    ...
    void printVertex(void)
      {
	std::cout<<"Vertex has neighbors"<< vert, degree;
       // printf("Vertex %d has %d neighbors ( ",vert,degree);
    ...
    void removeNbr(int y)
      {
        int i;
        for (i=0; i<degree; i++) if ((*nbrs)[i] == y) break;
        // we are supposed to be connected with vertex y and have to remove it
        if (i<degree)
          {
            if (i<degree-1) (*nbrs)[i] = (*nbrs)[degree-1];
            nbrs->pop_back();
            degree--;
          }
      } 
    ...
    void readCSV (string fname, int& vStart)
      {
        ...
        if (infile == NULL)
          {
	    std::cout<<"File %s does not exist"<< fname.c_str();
        ...
        if (fgets(line,100,infile)==NULL)
          {
	    std::cout<<"File  is empty."<< fname.c_str();
    ...
    Graph(string fname, int& vStart)
      {
        ...
        if (infile == NULL)
          {
            std::cout<<"File does not exist."<< fname.c_str();
            //printf("File %s does not exist.\n",fname.c_str());
            exit(2);
          }
        // Read the firfst header line
        if (fgets(line,100,infile)==NULL)
          {
	    std::cout<<"File %s is empty."<< fname.c_str();
           // printf("File %s is empty.\n",fname.c_str());
            exit(3);
          }
        i=0;
            ...
            if ( (int)line[0] != 0)
              {
	            ...
	            std::cout<<"Bad edge definition:."<< line<<u<<v;
                    //printf("Bad edge definition: %s : %d, %d\n",line,u,v);
                    fclose(infile);
                    adj.clear();
                    degen.clear();
                    exit(5);
        ...
        if (i != eCount)
          {
	    std::cout<<"Error reading file Wrong number of edges from ."<< fname.c_str()<<i<<eCount;
           // printf("Error reading file %s. Wrong number of edges: %d from %d\n",fname.c_str(),i,eCount);
            adj.clear();
            degen.clear();
            exit(10);
          }
    ...
    void removeV (deque<Vertex> * L, Vertex x)
      {
        ...
        int vv=x.getV();
        int sz = (*L).size();
        while ((i<sz)&&((*L)[i].getV()!=vv)) i++;
        if (i<sz)
          {
            if (i<sz-1)
              {
                (*L)[i].setV((*L)[sz-1].getV());
                (*L)[i].setNbrs(*((*L)[sz-1].getNbrs()));
              }
            L->pop_back();
          }
      }
...
{
    string f_name;
    int alg_sel=0,start_vert=0;
    char c;
    if (argc<2) usage();
    /* Parse command line arguments */ 
    while((c = getopt(argc, argv, "a:f:s:")) != -1)
    {
        switch(c)
        {
            case 'a':
